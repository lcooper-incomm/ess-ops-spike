...
@Component ( {
  selector: 'cca-financial-gift-search-results-table',
  templateUrl: './financial-gift-search-results-table.component.html',
  styleUrls: [ './financial-gift-search-results-table.component.scss' ]
} )
export class FinancialGiftSearchResultsTableComponent extends AbstractSearchResultsTableComponent implements OnInit {

  dataSource                           = new MatTableDataSource<Card> ();
  displayedColumns: string[]           = [ 'serialNumber', 'van', 'pin', 'pan', 'upc', 'description', 'vendorId', 'activationDate', 'owner', 'status', 'amount' ];
  hasLocationSearchPermission: boolean = false;
  hasUnmaskPanPermission: boolean      = false;
  ...
  ngOnInit () {
    ...
    this.dataSource.sortingDataAccessor = ( card: Card, property: string ) => {
      ...
      switch ( property ) {
        ...
        case 'amount':
          ...
          break;
        case 'status':
          let status = card.getStatusByPlatform ( PlatformType.GREENCARD );
    ...
    this.hasUnmaskPanPermission         = this.securityService.hasPermission ( Permission.UNMASK_PAN );
    ...
    if ( this.textFilter ) {
      ...
      return _.filter ( cards, function ( card: Card ) {
        ...
        return (card.identifiers.serialNumber && card.identifiers.serialNumber.indexOf ( filter ) !== -1)
          ...
          || (card.identifiers.panMasked && card.identifiers.panMasked.indexOf ( filter ) !== -1)
